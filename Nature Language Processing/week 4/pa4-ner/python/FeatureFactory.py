...
class FeatureFactory:
    ...
    def __init__(self):
        self.nameDic = set()
        instream = open('./name.txt','r')
        for name in instream:
            self.nameDic.add(name.split()[0])
	...
	if position > 1:
            pre = words[position - 1]
            features.append("preWord=" + pre)
            if pre in self.nameDic:
                features.append("feature=PreDefault")
            if pre.isupper():
                features.append("feature=PreTitle")
            if pre.isupper() and ('.' in pre or '\'' in pre):
                features.append("feature=PreAbbreviation")
            if pre == pre.capitalize():
                features.append("feature=PreCapitalize")
                
        if position + 1 < len(words):
            pos = words[position + 1]
            features.append("nextWord=" + pos)
            if pos in self.nameDic:
                features.append("feature=PosDefault")
            if pos.isupper():
                features.append("feature=PosTitle")
            if pos.isupper() and ('.' in pos or '\'' in pos):
                features.append("feature=PosAbbreviation")
            if pos == pos.capitalize():
                features.append("feature=PosCapitalize")
            
        if currentWord in self.nameDic:
            features.append("feature=DefaultName")
            features.append("Default=" + currentWord)
        ...
        if currentWord[0].isupper() and ('.' in currentWord or '\'' in currentWord) :
            features.append("feature=Abbreviation")
