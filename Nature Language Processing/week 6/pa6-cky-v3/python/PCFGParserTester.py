...
class PCFGParser(Parser):
    ...
    def train(self, train_trees):
        ...
        binarized_trees = [TreeAnnotations.annotate_tree(atree) for atree in train_trees]
        self.lexicon = Lexicon(binarized_trees)
        self.grammar = Grammar(binarized_trees)
    ...
    def get_best_parse(self, sentence):
        ...
        self.back = []
        nonTerm = self.lexicon.get_all_tags()
        
        for i in range(0, wordNum + 1):
            self.score.append([])
            self.back.append([])
            for j in range(0, wordNum + 1):
                self.score[i].append(collections.defaultdict(lambda: 0.0))
                self.back[i].append(collections.defaultdict(lambda: 0.0))

        for i in range(0, wordNum):
            for A in nonTerm:
                self.score[i][i + 1][A] = self.lexicon.score_tagging(sentence[i],A)
            added = True
            while added:
                added = False
                for item in self.grammar.unary_rules_by_child.keys():
                    for unary_rule in self.grammar.unary_rules_by_child[item]:
                        A = unary_rule.parent
                        B = unary_rule.child
                        if self.score[i][i + 1][B] > 0:
                            prob = unary_rule.score * self.score[i][i + 1][B]
                            if prob > self.score[i][i + 1][A]:
                                self.score[i][i + 1][A] = prob
                                self.back[i][i + 1][A] = B
                                added = True

        for span in range(2, wordNum + 1):
            for begin in range(0, wordNum - span + 1):
                end = begin + span
                for split in range(begin + 1, end):
                    for item in self.grammar.binary_rules_by_left_child.keys():
                        for binary_rule in self.grammar.binary_rules_by_left_child[item]:
                            A = binary_rule.parent
                            B = binary_rule.left_child
                            C = binary_rule.right_child
                            prob = self.score[begin][split][B] * self.score[split][end][C] * binary_rule.score
                            if prob > self.score[begin][end][A]:
                                self.score[begin][end][A] = prob
                                self.back[begin][end][A] = (split,B,C)
                added = True
                while added:
                    added = False
                    for item in self.grammar.unary_rules_by_child.keys():
                        for unary_rule in self.grammar.unary_rules_by_child[item]:
                            A = unary_rule.parent
                            B = unary_rule.child
                            prob = unary_rule.score * self.score[begin][end][B]
                            if prob > self.score[begin][end][A]:
                                self.score[begin][end][A] = prob
                                self.back[begin][end][A] = B
                                added = True
   
        return TreeAnnotations.unannotate_tree(self.buildTree(0,wordNum,'ROOT'))
