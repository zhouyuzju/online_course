...
class Googling:
    ...
    def guessLocation(self, data):
        ...
        rank = 1
        for q in data:
            locs = re.findall('<LOCATION>(.*?)</LOCATION>', q.title)
            locs.extend(re.findall('<LOCATION>(.*?)</LOCATION>', q.snip))

            
            for loc in locs:
                if loc in self.cities:
                    if loc not in cityCount:
                        cityCount[loc] = 0.0
                    cityCount[loc] += (1.0 / rank)
                for k, v in self.countries.items():
                    if loc == v:
                        if loc not in countryCount:
                            countryCount[loc] = 0.0
                        countryCount[loc] += (1.0 / rank)
                        break
            rank += 1
        maxv = 0
        cityAns = ''
        ...
        for k, v in cityCount.items():
            infos = self.cities[k]
            for info in infos:
                cname = self.countries[info[0]]
                
                if cname in countryCount and v + countryCount[cname] > mscore:
                    mscore = v + countryCount[cname]
                    #print k, cname
                    cityAns = k
                    countryAns = cname
                    if countryAns == 'United States':
                        countryAns = self.states[info[2]]
                    
            if v > maxv:
                maxv = v
                cityAns = k
        if cityAns != '' and countryAns != '':
            return Location(cityAns.lower(), countryAns.lower())
        
        if cityAns:
            infos = self.cities[cityAns]
            if len(infos) == 1:
                countryAns = self.countries[infos[0][0]]
                if countryAns == 'United States':
                    countryAns = self.states[infos[0][2]]
                #print "%s-%s" % (cityAns, countryAns)
            else:    
                for info in infos:
                    cname = self.countries[info[0]]
            ...
            if countryAns == '':
                ...
                for info in infos:
                    ...
                    if popu > maxp:
                        maxp = popu
                        countryAns = self.countries[info[0]]
                        if countryAns == 'United States':
                            countryAns = self.states[info[2]]
        else:
            maxv = 0
    ...
    def processQueries(self, queryData):
        ...
        f = open('../data/CountryInfo.txt')
        self.countries = {}
        for line in f:
            if line[0] == '#':
                continue
            info = line.split('\t')
            cc, country = info[0], info[4]
            self.countries[cc] = country
        f.close()

        f = open('../data/cities15000.txt')
        self.cities = {}
        for line in f:
            info = line.split('\t')
            name, asciiname, alternatenames, cc, popu, state = info[1], info[2], info[3], info[8], long(info[14]), info[10]
            #if name == 'Moscow':
            #    print line
            #    print cc
            names = [name, asciiname]
            #names.extend(alternatenames.split(','))
            for name in names:
                if name not in self.cities:
                    self.cities[name] = []
                self.cities[name].append((cc, popu, state))
        f.close()
        self.states = {}
        f = open('../data/stats.txt')
        for line in f:
            line = line.strip()
            info = line.split('\t')
            #print info
            state, abbr = info[0], info[1]
